    ...
    foreach ($File in $Files)
    {
        ...
        foreach ($MarkdownLine in $MarkDownLines)
        {
            ...
            if ($YamlDividingLines -eq 1)
            {
                ...
                $HugoNameAndValue = get-HugoNameAndOneLineValue -FrontMatterLine $Line
                [string]$PropertyName = $HugoNameAndValue.name
                [string]$PropertyValue = $HugoNameAndValue.Value
